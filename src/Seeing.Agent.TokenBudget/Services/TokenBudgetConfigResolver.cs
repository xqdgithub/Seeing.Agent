...
public class TokenBudgetConfigResolver : ITokenBudgetConfigResolver
{
    ...
    /// </summary>
    public TokenBudgetConfig ResolveWithModelLimits(
        TokenBudgetConfig? sessionConfig,
        TokenBudgetConfig? agentConfig,
        TokenBudgetConfig? globalConfig,
        int? modelContextLimit)
    {
        // 1. Use existing Resolve logic to get base configuration
        var result = Resolve(sessionConfig, agentConfig, globalConfig);
        
        // 2. Apply model limit as upper bound constraint
        if (modelContextLimit.HasValue)
        {
            if (!result.MaxContextTokens.HasValue)
            {
                // No config set, use model limit
                result.MaxContextTokens = modelContextLimit.Value;
            }
            else if (result.MaxContextTokens.Value > modelContextLimit.Value)
            {
                // User config exceeds model limit, constrain to model limit
                result.MaxContextTokens = modelContextLimit.Value;
            }
            // Otherwise: user's smaller value wins
        }
