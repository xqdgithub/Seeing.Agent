...
public interface ISeeingModule
{
    ...
    Task ActivateAsync(IServiceProvider services, CancellationToken cancellationToken = default);

    /// <summary>停用：对称卸载工具、断开连接、撤销 Handler</summary>
    Task DeactivateAsync(IServiceProvider services, CancellationToken cancellationToken = default);
}
