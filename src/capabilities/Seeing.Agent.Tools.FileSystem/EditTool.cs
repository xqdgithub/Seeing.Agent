...
namespace Seeing.Agent.Core.Tools.FileSystem
{
    ...
    public class EditTool : ToolBase
    {
        ...
        /// 创建 EditTool 实例
        /// </summary>
        public EditTool(ILogger<EditTool> logger, IFileSystem fileSystem) : base(logger)
        {
        ...
        public override async Task<ToolResult> ExecuteAsync(JsonElement arguments, ToolContext context)
        {
            ...
            // 读取文件内容
            var oldContent = await _fileSystem.ReadAllTextAsync(filePath, context.CancellationToken);
            ...
            try
            {
                await _fileSystem.WriteAllTextAsync(filePath, newContent, context.CancellationToken);
            }
            catch (Exception ex)
            {
