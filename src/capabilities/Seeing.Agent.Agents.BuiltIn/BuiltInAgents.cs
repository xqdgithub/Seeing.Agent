...
public static class BuiltInAgents
{
    ...
    /// 获取所有内置 Agent 定义
    /// </summary>
    public static IEnumerable<AgentDefinition> GetBuiltInAgents()
    {
    ...
    /// <summary>
    /// 创建 build Agent - 默认主代理（无 Allowed/Denied，工具全开）
    /// </summary>
    private static AgentDefinition CreateBuildAgent()
    {
...
- 探索代码库时优先委托子代理
- 需要向用户提问或澄清时，若 question 工具可用则优先调用它提出结构化问题（单选/多选/文本），而不是在回复正文里直接追问
- 仅当 question 工具不可用或无交互环境时，才改用文本提问
...
""",
            ...
            Tags = ["primary", "full-access", "default"]
        };
    }

    /// <summary>
    /// 创建 plan Agent - 计划模式（AllowedTools 白名单）
    /// </summary>
    private static AgentDefinition CreatePlanAgent()
    {
        return new AgentDefinition
        {
            Name = "plan",
...
""",
            ...
            Tags = ["planning", "readonly", "safe", "structured"]
        };
    }
    ...
    /// </summary>
    private static AgentDefinition CreateExploreAgent()
    {
        return new AgentDefinition
        {
            Name = "explore",
...
""",
    ...
    /// </summary>
    private static AgentDefinition CreateGeneralAgent()
    {
        return new AgentDefinition
        {
            Name = "general",
...
""",
            ...
            PermissionDefaultEffect = PermissionEffect.Ask,
            Tags = ["general", "research", "multistep"]
        };
    }
    ...
    /// </summary>
    private static AgentDefinition CreateSummaryAgent()
    {
        return new AgentDefinition
        {
            Name = "summary",
...
严格按以下 Markdown 结构输出，保持章节顺序，如果其他重要内容，在后面分节写入，不要输出模板标记：
...
""",
            Tags = ["system", "hidden", "summary-generation"]
        };
    }
}
