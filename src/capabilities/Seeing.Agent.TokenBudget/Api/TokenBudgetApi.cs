...
namespace Seeing.Agent.TokenBudget.Api;
...
public class TokenBudgetApi : ITokenBudgetApi
{
    ...
    /// <param name="compressionService">压缩编排服务（压缩唯一执行入口）</param>
    ...
    {
        var session = await _sessionManager.GetOrLoadAsync(sessionId);
        session.BudgetConfig = config ?? throw new ArgumentNullException(nameof(config));
        session.UpdatedAt = DateTime.Now;
        await _sessionManager.SaveAndNotifyAsync(sessionId, persist: true);
    }
    ...
    private async Task<SessionData> GetSessionOrThrowAsync(string sessionId)
    {
        ...
        try
        {
            return await _sessionManager.GetOrLoadAsync(sessionId);
        }
