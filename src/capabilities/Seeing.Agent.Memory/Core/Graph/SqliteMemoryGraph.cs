...
public class SqliteMemoryGraph : IMemoryGraph
{
    ...
    public SqliteMemoryGraph(SqliteConnectionOwner owner, ILogger<SqliteMemoryGraph>? logger = null)
        : this(owner, new SqliteConnectionGate(), logger)
    {
    }
    ...
    public Task<GraphStats> GetStatsAsync(CancellationToken ct = default) =>
        _gate.RunAsync(async token =>
        {
            ...
            using var isolatedCmd = Connection.CreateCommand();
