...
public sealed class SessionReloadHandler : ReloadHandlerBase<WorkspaceChange>
{
    ...
    private readonly ILogger<SessionReloadHandler>? _logger;
    ...
    public SessionReloadHandler(
        ISessionStore store,
        SessionManager sessionManager,
        ILogger<SessionReloadHandler>? logger = null)
    {
        _store = store;
    ...
    protected override async Task ReloadAsync(WorkspaceChange change, CancellationToken ct)
    {
        ...
        if (_store is IPersistenceFlusher flusher)
        {
            ...
            {
                _logger?.LogWarning(ex, "工作区切换前会话落盘失败，继续重定位");
            }
        }
