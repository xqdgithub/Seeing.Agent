...
- 输入行渲染 `> 文本▌`：`▌` 是**插入点标记**，光标可落在文本中间（`←/→` 移动，`Backspace/Delete` 删除）。
- 每帧写完后，终端**物理光标**会被移到插入点上（下一帧渲染前再放回活动区末行）：
  终端把输入法组合串画在物理光标处，不定位就会跑到活动区末行（状态栏那一行），表现为「输入法框不在光标位置」。
  定位按**显示格**计算（CJK 记 2 格），与 `▌` 标记所在列一致。
- 输入行被高度预算裁掉（终端过矮、多行输入超出行数上限）时不做定位，光标留在活动区末行。
- **空闲不重绘**：周期补绘（60ms）只在「确有可见变化」时写终端（内容/执行态/后台执行/待批/用量·审批·工作目录/终端尺寸/固化提交），
  避免空闲时反复擦写整帧把输入法组合串擦掉（闪屏）；输入、命令、事件阈值命中仍走强制重绘。
- 每帧只写一次（`Live` 的更新目标自身已刷新），不重复写整帧。
