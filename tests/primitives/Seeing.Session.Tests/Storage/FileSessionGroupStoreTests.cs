...
public class FileSessionGroupStoreTests : IDisposable
{
    ...
    public async Task SaveAndLoad_ShouldRoundTrip()
    {
        ...
        await store.SaveAsync(MakeGroup("grp_1", "s1", "s2"));

        var loaded = await store.LoadAsync("grp_1");
    ...
    public async Task LoadAsync_WhenMissing_ShouldReturnNull()
    {
        ...
        (await store.LoadAsync("grp_none")).Should().BeNull();
    }
    ...
    public async Task FindBySessionAsync_ShouldMatchAnyMember()
    {
        var store = new FileSessionGroupStore(_dir);
        await store.SaveAsync(MakeGroup("grp_1", "s1", "s2"));
        await store.SaveAsync(MakeGroup("grp_2", "s3"));

        var found = await store.FindBySessionAsync("s2");
        var anchor = await store.FindBySessionAsync("s3");

        found.Should().NotBeNull();
        found!.Id.Should().Be("grp_1");
        anchor.Should().NotBeNull();
        anchor!.Id.Should().Be("grp_2");
        (await store.FindBySessionAsync("missing")).Should().BeNull();
    }
    ...
    {
        var store = new FileSessionGroupStore(_dir);
        await store.SaveAsync(MakeGroup("grp_1", "s1"));

        await store.DeleteAsync("grp_1");

        (await store.LoadAsync("grp_1")).Should().BeNull();
    }
    ...
    public async Task SetBaseDirectory_ShouldRelocateReadsAndWrites()
    {
        ...
        {
            var store = new FileSessionGroupStore(_dir);
            await store.SaveAsync(MakeGroup("grp_old", "s1"));

            store.SetBaseDirectory(dir2);
            store.BaseDirectory.Should().Be(dir2);

            (await store.LoadAsync("grp_old")).Should().BeNull();

            await store.SaveAsync(MakeGroup("grp_new", "s2"));
            (await store.LoadAsync("grp_new")).Should().NotBeNull();
        }
        finally
        {
