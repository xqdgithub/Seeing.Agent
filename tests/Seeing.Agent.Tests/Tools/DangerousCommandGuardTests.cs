...
public class DangerousCommandGuardTests
{
    ...
    [InlineData("sudo -u root rm -rf ./node_modules")]
    ...
    [InlineData("sudo --preserve-env rm -rf /")]
    [InlineData("sudo --preserve-env=FOO rm -rf /")]
    [InlineData("sudo -p 'secret' rm -rf /")]
    [InlineData("sudo -T 5 rm -rf /")]
    [InlineData("sudo -uroot rm -rf /")]
