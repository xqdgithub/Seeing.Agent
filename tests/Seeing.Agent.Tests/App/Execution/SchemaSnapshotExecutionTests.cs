...
public class SchemaSnapshotExecutionTests
{
    ...
    {
        var logger = NullLogger<Seeing.Agent.Tools.ToolManager>.Instance;
        var hooks = new Seeing.Agent.Core.Hooks.HookManager(
            NullLogger<Seeing.Agent.Core.Hooks.HookManager>.Instance);
        var manager = new Seeing.Agent.Tools.ToolManager(logger, hooks);
        ...
        bool withCatalog)
    {
        ...
        var toolManager = new Seeing.Agent.Tools.ToolManager(
