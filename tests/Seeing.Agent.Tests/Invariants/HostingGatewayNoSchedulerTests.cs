...
/// <summary>
/// Host Shape：Hosting.Gateway 不得经 Gateway 传递引用 Scheduler 能力包。
/// </summary>
public class HostingGatewayNoSchedulerTests
{
    [Fact]
    public void Hosting_Gateway_Transitive_ProjectReferences_Should_Not_Include_Scheduler()
    {
        ...
        projectNames.Should().Contain("Seeing.Agent.Gateway");
        projectNames.Should().NotContain("Seeing.Agent.Scheduler",
            "Hosting.Gateway → Gateway 不得再传递引用 Scheduler；cron 由 sample 单独 AddSeeingScheduler");
    }
