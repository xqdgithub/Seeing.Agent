...
public class SchemaScenarioTests
{
    ...
    public async Task InFlightBoundary_ProcessReloadDefersDeactivate_SessionSwitchOnlyAffectsNextSubmit()
    {
        ...
        var lifecycle = new ModuleLifecycleManager(catalog, [a, b], NullLogger<ModuleLifecycleManager>.Instance);
        ...
        var handler = new ModuleSettlementReloadHandler(
            ...
            options,
            [a, b],
            new ModuleReloadOptions { ForceCancelInFlight = false },
    ...
    private static Seeing.Agent.Tools.ToolManager CreateToolManager(params string[] ids)
    {
        var manager = new Seeing.Agent.Tools.ToolManager(
            NullLogger<Seeing.Agent.Tools.ToolManager>.Instance,
        ...
        IReadOnlyList<TrackingModule>? modules = null)
    {
        ...
        if (modules is { Count: > 0 })
        {
            ...
            lifecycle = new ModuleLifecycleManager(
                catalog, modules, NullLogger<ModuleLifecycleManager>.Instance);
