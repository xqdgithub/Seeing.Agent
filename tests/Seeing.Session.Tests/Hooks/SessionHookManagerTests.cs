...
namespace Seeing.Session.Tests.Hooks
{
    public class SessionHookManagerTests
    {
        ...
        public void AddHook_ShouldAddHookToList()
        {
            ...
            hook.Setup(h => h.HookPoint).Returns(HookPoints.Created);
            hook.Setup(h => h.Priority).Returns(10);

            manager.AddHook(hook.Object);

            manager.GetHookCount(HookPoints.Created).Should().Be(1);
        }
        ...
        public void AddHook_ShouldIgnoreNullHook()
        {
            ...
            manager.GetHookCount(HookPoints.Created).Should().Be(0);
        }
        ...
        public void AddHook_ShouldSupportMultipleHookPoints()
        {
            ...
            createdHook.Setup(h => h.HookPoint).Returns(HookPoints.Created);
            createdHook.Setup(h => h.Priority).Returns(10);

            var savedHook = new Mock<ISessionHook>();
            savedHook.Setup(h => h.HookPoint).Returns(HookPoints.Saved);
            ...
            manager.GetHookCount(HookPoints.Created).Should().Be(1);
            manager.GetHookCount(HookPoints.Saved).Should().Be(1);
        }
        ...
        public void ClearHooks_ShouldRemoveAllHooksForPoint()
        {
            ...
            manager.AddHook(hook1.Object);
            manager.AddHook(hook2.Object);
            var result = manager.ClearHooks(HookPoints.Created);
        ...
        {
            HookPoints.Created.Should().Be("session.created");
            HookPoints.Saving.Should().Be("session.saving");
            HookPoints.Saved.Should().Be("session.saved");
            HookPoints.Loading.Should().Be("session.loading");
            HookPoints.Loaded.Should().Be("session.loaded");
            HookPoints.Destroyed.Should().Be("session.destroyed");
        }
