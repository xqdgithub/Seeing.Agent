...
public class SchedulerToolRegistrationTests
{
    ...
    [Fact]
    public void AddSeeingSchedulerTools_AfterBuiltInITool_ShouldRegisterConcreteCronToolsNotAsITool()
    {
        var services = new ServiceCollection();
        services.AddSingleton<ITool>(new StubTool("read"));
        services.AddSeeingSchedulerTools();

        // Activate 挂载：DI 只登记具体类型，不登记 ITool（由 SchedulerModule.Activate 挂到 IToolManager）
        ...
        services.Where(d => d.ServiceType == typeof(ITool)).Should().HaveCount(1,
            "cron tools must not be registered as ITool in DI; Activate mounts them");
    }
