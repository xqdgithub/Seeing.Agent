...
| 页级导航保留 | 首页/标题/状态徽标/工作区徽标/大屏入口/新建会话 |
...
- `<Header>` 模板精简为页级导航区：首页按钮、会话标题、状态徽标、工作区徽标、大屏入口（VideoCamera）、新建会话
...
| 用例 | 预期 |
|------|------|
| SessionToolbar Full 模式渲染工具栏 | 显示 Agent/模型选择器与操作按钮 |
| SessionToolbar Summary 模式 | 不渲染工具栏 |
| 子会话只读视图 | 清空/重命名禁用，显示"返回主会话" |
| ACP 透传 | 显示 Mode/Model 输入框 |
| 模型选择变更 | 调用 `Context.SetModelAsync` |
...
- T4 大屏点击 Summary 切换主窗口 → 工具栏随激活会话变化
- T5 工具栏清空会话 → 子会话连带删除
- T6 回归：既有 115 项单测全绿
